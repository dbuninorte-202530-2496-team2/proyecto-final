...
const RegistroClasesTab: React.FC = () => {
  ...
  // Estado de sesión
  const [tutorActivoId, setTutorActivoId] = useState<TutorFilter>(() => {
    if (rol === 'TUTOR' && usuario?.id) {
      return usuario.id;
    }
    return 'none';
  });

  // Efecto para actualizar el tutor activo si cambia el usuario/rol (por si acaso)
  useEffect(() => {
    if (rol === 'TUTOR' && usuario?.id) {
      setTutorActivoId(usuario.id);
    }
  }, [rol, usuario]);
